...
function selfAssign()
{
	...
	$stmt = $db->prepare("SELECT callsign, id FROM " . DB_PREFIX . "active_users WHERE identifier = ?");
	$stmt->bindValue('?', $unit);
	$result = $stmt->execute();
	if ($result === false) {
	...
	$stmt = $db->prepare("INSERT INTO " . DB_PREFIX . "calls_users (call_id, identifier, callsign, id) VALUES (?, ?, ?, ?)");
	$result = $stmt->execute(array($callId, $unit, $callsign, $id));
	if ($result === false) {
		die(print_r($db->errorInfo(), true));
	}
	$stmt = $db->prepare("UPDATE " . DB_PREFIX . "active_users SET status = '0', status_detail = '3' WHERE " . DB_PREFIX . "active_users.callsign = ?");
	$result = $stmt->execute(array($callsign));
	if ($result === false) {
		die(print_r($db->errorInfo(), true));
	}
	//Now we'll add data to the call log for unit history
	$narrativeAdd = date("Y-m-d H:i:s") . ': Unit: ' . $callsign . ' self-assigned to the call. <br/>';

	$stmt = $db->prepare("UPDATE " . DB_PREFIX . "calls SET call_narrative = concat(call_narrative, ?) WHERE call_id = ?");
	$result = $stmt->execute(array($narrativeAdd, $callId));
